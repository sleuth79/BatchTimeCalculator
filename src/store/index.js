  ...
  <div class="start-time-results">
    ...
    <p v-if="showDetailedResults && displayFinalPosition">
      ...
      <span class="result-value">
        <template v-if="displayedCandidate && typeof displayedCandidate === 'object'">
          {{ displayedCandidate.position }} :
          {{ displayedCandidate.startTime || displayBatchStartTime }} to 
          {{ displayedCandidate.endTime }}
        </template>
        <template v-else>
...
export default {
  ...
  setup(props) {
    ...
    // Check that the batch start time is in hh:mm format.
    const showDetailedResults = computed(() => /^\d{2}:\d{2}$/.test(displayBatchStartTime.value));
    ...
    // Legacy fallback for closest position display.
    ...
    // NEW: Use the computed getter from the store for the displayed candidate.
    const displayedCandidate = computed(() => gcStore.displayedClosestCandidate);
    ...
    return {
      ...
      showStartTimeFinalPosition: computed(() => true),
      showDetailedResults,
      displayedCandidate,
