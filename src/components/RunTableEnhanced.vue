<template>
    <div class="run-table">
      <table>
        ...
        <tbody>
          <!-- Initial Batch Rows -->
          <tr
            v-for="(run, index) in computedRuns"
            :key="'initial-' + index"
            v-if="computedRuns.length > 0"
          >
  ...
  <script>
  // Helper function for parsing runtime strings
    ...
    computed: {
      ...
      additionalRows() {
        ...
        const additionalCount =
          timeDelayResults && timeDelayResults.additionalRuns ? timeDelayResults.additionalRuns : 0;
      ...
      prebatchRows() {
        const { startTime, allGcData, selectedGc, timeDelayResults } = this.gcStore;
        const prebatchCount =
          timeDelayResults && timeDelayResults.totalDelayedRuns ? timeDelayResults.totalDelayedRuns : 0;
