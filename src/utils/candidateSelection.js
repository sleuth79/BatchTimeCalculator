...
// This module provides a function to perform candidate selection for the run table.
// It uses helper functions to adjust raw run numbers based on control values,
// generate a full order and sample order, and finally select the appropriate candidate.

export function getDisplayedPosition(raw, controls) {
    ...
    if (raw < 17) {
      ...
      if (sample === control1 || sample === control2) {
        console.log(`[getDisplayedPosition] Raw ${raw}: base sample ${sample} equals a control; adjusting downward.`);
        sample = sample - 1;
      }
    } else {
      sample = raw;
      if (raw === control1 || raw === control2) {
        if (raw < 23) {
          console.log(`[getDisplayedPosition] Raw ${raw}: equals a control in lower block; adjusting upward.`);
          sample = raw + 1;
        } else {
          console.log(`[getDisplayedPosition] Raw ${raw}: equals a control in higher block; adjusting downward.`);
          sample = raw - 1;
        }
      }
    }
    console.log(`[getDisplayedPosition] Final displayed sample for raw ${raw} = ${sample}`);
    return sample;
  }
  
  export function generateSampleAllowed(finalPos, controls) {
    const arr = [];
  ...
  export function generateFullOrder(finalPos, gcType, controls) {
    const order = [];
    order.push("Blank");
  ...
  export function extractSamplePositions(fullOrder) {
    return fullOrder.filter(item => item.startsWith("Position "));
  }
  
  /**
   * selectCandidate performs the candidate selection.
   * @param {Array} runs - Array of run objects (from calcResults.runs)
   * @param {Object} controls - Object with control1 and control2 values.
   * @param {number} finalPosition - The final sample position (a number).
   * @param {string} gcType - The GC type (e.g., "energy" or "sulphur") in lowercase.
   * @returns {Object} - An object with the candidate run and related debug info.
   */
  export function selectCandidate(runs, controls, finalPosition, gcType) {
    const todayStr = new Date().toDateString();
    // Set cutoff time to 16:00:00 today (4:00 PM)
    const cutoff = new Date(`${todayStr} 16:00:00`);
    console.log(`[selectCandidate] Cutoff time:`, cutoff);
    
    const fullOrder = generateFullOrder(finalPosition, gcType, controls);
